...
def answers_to_db(headline_id, username, answer):
    ...
    result = db.session.execute(
        sql, {"headline_id": headline_id, "username": username, "answer": answer})
    db.session.commit()
...
def opinions(headline_id, username, opinion):
    ...
    result = db.session.execute(
        sql, {"headline_id": headline_id, "username": username, "opinion": opinion})
    db.session.commit()
    return result.scalar()
...
def send(username, message_text, headline, answer, opinion):
    ...
    try:
        db.session.execute(sql, {"message_text": message_text,
                           "user_id": user_id, "headline_id": headline_id, "answer": answer})
        db.session.commit()
        return True
